...
function CalendarGearIcon() {
  return (
    <span className="relative inline-flex h-10 w-10 text-amber-700">
      <svg
        aria-hidden="true"
        className="h-10 w-10"
        fill="none"
      ...
      <svg
        aria-hidden="true"
        className="absolute -bottom-1 -right-1 h-5 w-5 rounded-full bg-amber-50 p-0.5"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={1.75}
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-1 flex-col justify-center px-4 py-12 sm:py-16">
        ...
        <ul className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {CARDS.map((card) => {
            ...
            return (
              <li key={card.title}>
                <article className="flex min-h-[22rem] flex-col items-center justify-center rounded-3xl border border-zinc-200 bg-white p-8 text-center shadow-sm sm:aspect-square sm:min-h-0">
